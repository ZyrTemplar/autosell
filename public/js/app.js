...
$(document).ready(function () {
  $.ajax({
    ...
    success: function success(data) {
      var temp = data[0].brand;

      for (var i = 0; i < data.length; i++) {
        if (data[i].brand === temp) continue;
        var item = '<option value="' + data[i].brand + '">' + data[i].brand + '</option>';
        $('#brand').append(item);
        temp = data[i].brand;
      }
    },
...
window.change_menu = function (id) {
  ...
  if (id === 'change') {
    $('.configs').html('<div class="changer">' + '<form action="/change" method="POST" class=" h-100 text-center pl-5 col">\n' + '<input type="hidden" id="csrf" name="_token" value= "<?php{{ csrf_token() }}>" />' + '<h1 class="mb-5">Змініть потрібні дані</h1>' + '<div class="form-group text-center w-75 mx-auto">\n' + '<h2><label for="name">Ваше ім\'я</label></h2>' + '<input type="text" class="form-control" id="name" name="name">\n' + '</div>' + '<div class="form-group text-center w-75 mx-auto">\n' + '<h2><label for="email">Ваш Email</label></h2>\n' + '<input type="email" class="form-control " id="email" name="email">\n' + '</div>' + '<div class="form-group text-center w-75 mx-auto">\n' + '<h2><label for="number">Ваш мобільний номер</label></h2>\n' + '<input type="text" class="form-control " id="number" name="number">\n' + '</div>' + '<button type="submit" class="btn btn-primary mt-3">Змінити дані</button>' + '</form>' + '</div>');
    $.ajax({
      url: "/ajax/getUser",
      method: 'GET',
      data: {
...
$(function () {
  var max_file_number = 10,
      ...
      $button = $('#submit_auto', $form); // Disable submit button on page ready.
